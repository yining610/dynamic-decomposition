...
class DocDB(object):
    ...
    def build_db(self, db_path, data_path):
        ...
        tokenizer = RobertaTokenizer.from_pretrained("roberta-large", cache_dir=MODEL_CACHE_DIR)
...
class Retrieval(object):
    ...
    def __init__(self, db, cache_path, embed_cache_path,
                 retrieval_type="gtr-t5-large", batch_size=None, device="cuda"):
        self.db = db
    ...
    def load_encoder(self):
        ...
        encoder = SentenceTransformer("sentence-transformers/" + self.retrieval_type, cache_folder=MODEL_CACHE_DIR)
    ...
    def get_gtr_passages(self, topic, retrieval_query, passages, k):
        ...
        if topic in self.embed_cache:
            ...
            passage_vectors = self.encoder.encode(inputs, batch_size=self.batch_size, device=self.encoder.device, show_progress_bar=False)
