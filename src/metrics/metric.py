"""Adapted from https://github.com/OpenLMLab/MOSS-RLHF/blob/main/metric.py"""
...
class Metrics():
    ...
    def write_wandb(self, global_step, gathered_metrics: Dict[str, float] = None):
        ...
        results = {f'{self.mode}/{k}': v for k, v in results.items()}
        self.logger.log(results, step=global_step)

    def display(self, global_step, data_size = None, gathered_metrics: Dict[str, float] = None, eval=False):
        ...
        if eval:
            ...
            if data_size is not None and 'global_num_examples' in results:
                print(f"=========== Step: {global_step}, Epoch: {(results['global_num_examples'] / data_size):.2f} ===========")
            else:
